...
<body>
	...
	<p>
		Les listes vous permettent de retrouver facilement les produits que
		vous souhaitez acheter. <br> Vous pouvez créer jusqu'à 10 listes
		différentes de 100 articles maximum.
	</p>
		...
		<c:if test="${requestScope.categorie == 'UTILISATEUR'}">
			<ul>
				<c:forEach var="list" items="${shoppingLists}" varStatus="status">
					<li>
						<p class="btn" data-bs-toggle="modal"
							data-bs-target="#modalProduits"
							onclick="chargerProduitsListe(${list.id}, '${list.name}')">
							${status.index + 1} - ${list.name}</p>
						<div id="btn-Memo_container-${list.id}">
							<!--Contenu généré par JS JS -->
						</div> <a
						href="servletListeCourse?type_action=delete_list&list_id=${list.id}">
							<img src="recupererImage?cheminImage=icons/delete_icon.png"
							class="btn-DeleteListe" Title="Supprimer la liste de course" />
					</a>
					</li>
				</c:forEach>
			</ul>
			<button type="button" class="btn-Add" data-bs-toggle="modal"
				data-bs-target="#modalAjout">Ajouter liste</button>
		</c:if>
	</div>

	<!-- Modal produits dans la liste de course -->
	<div class="modal fade" id="modalProduits" tabindex="-1"
		aria-labelledby="ModalProduitsLabel" aria-hidden="true">
			...
			<div class="modal-content">
				...
				<form id="formProduits" action="gestionProduitListe" method="post">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-convertir"
							onclick="convertirListeEnPanier()">Convertir en Panier</button>
	...
	<div class="modal fade" id="modalAjout" tabindex="-1"
		aria-labelledby="modalAjoutLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
	...
	<div class="modal fade" id="modalMemo" tabindex="-1"
		aria-labelledby="ModalMemoLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary"
						data-bs-dismiss="modal">Fermer</button>
					<button id="convertToProductsBtn" onclick="conversionMemos()"
						type="button" class="btn btn-primary">Convertir en
						produits</button>
				</div>
