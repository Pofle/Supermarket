...
<body>
	      ...
	      <form action="FinaliserCommande" method="post">
	      	<div class="modal-body">
        		...
        		<label for="date">Sélectionnez une date :</label>
        			<input type="date" id="date" name="date" onchange="updateCreneaux()">
        		<br><br>
    ...
    function updateCreneaux() {
        ...
        var creneaux = <%= request.getAttribute("creneaux") %>;
        
        // Vérifier si les créneaux ne sont pas nuls
        if (creneaux !== null) {
            // Nettoyer la liste des créneaux
            creneauSelect.innerHTML = "";
            
            // Ajouter les créneaux disponibles pour la date sélectionnée
            for (var i = 0; i < creneaux.length; i++) {
                var creneau = creneaux[i];
                if (creneau.jour.date === selectedDate) {
                    var option = document.createElement("option");
                    option.text = creneau.heureDebut + " - " + creneau.heureFin;
